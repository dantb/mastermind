...
trait MonadError[F[_]: Monad, E]:
  ...
  final def handleError[A](fa: F[A])(f: E => A): F[A] = handleErrorWith(fa)(e => Monad[F].pure(f(e)))
  def raiseError[A](e: E): F[A]
  def handleErrorWith[A](fa: F[A])(f: E => F[A]): F[A]

trait MonadErrorOps:
  extension [E, A, F[_]](fa: F[A])
    def handleErrorWith(f: E => F[A])(using MonadError[F, E]): F[A] = MonadError[F, E].handleErrorWith(fa)(f)
